#ifndef WATERLIGHTING
#define WATERLIGHTING
...
float D_GGX_UE4(float a2, float NoH)
{
    ...
    return a2 / max(0.000001, (PI * d * d));					// 4 mul, 1 rcp
...
float D_DistrubutionGGX(float3 N, float3 H, float a2, float Pi)
{
    ...
    //0.001是为了防止分母为0
    return nominator / max(0.000001, denominator);
}
...
///                           ///
/////////////////////////////////
float3 WaterDirectLightingBRDF(float3 DiffuseColor, float3 SpecularColor, float Roughness, float3 N, float3 V, float3 L, float3 LightColor, float Shadow)
{
...
///                             ///
///////////////////////////////////
float3 WaterIndirectLightingBRDF(float3 DiffuseColor, float3 SpecularColor, float Roughness, float3 positionWS, float3 N, float3 V,
float2 screenPos, float reflectIntensity, float mask, Texture2D PlanarReflectionTex, sampler PlanarReflectionSampler, float Occlusion)
{
    // ----------
    // SH内置 *PI
    ...
    // ---------------------
    // IBL[SplitSum - Part1]
    float2 uv_Planar = screenPos + lerp(float2(0.0, 0.0), N.xz, reflectIntensity);
    float3 specularLD = SAMPLE_TEXTURE2D(PlanarReflectionTex, PlanarReflectionSampler, uv_Planar).xyz * mask;
    // float3 specularLD = 1.0;
    // #ifdef REFLECTIONMODE_PROBE
    //     float3 rDir = normalize(reflect(-V, N));
    //     specularLD = GlossyEnvironmentReflection(rDir, positionWS, Roughness, Occlusion);
    // #elif REFLECTIONMODE_PLANAR
    //     float2 uv_Planar = screenPos + lerp(float2(0.0, 0.0), N.xz, reflectIntensity);
    //     // float mip = PerceptualRoughnessToMipmapLevel(Roughness);
    //     // specularLD = SAMPLE_TEXTURE2D_LOD(PlanarReflectionTex, PlanarReflectionSampler, uv_Planar, mip).xyz;
    //     specularLD = SAMPLE_TEXTURE2D(PlanarReflectionTex, PlanarReflectionSampler, uv_Planar).xyz;
    // #endif
    // -------------------------
    // 数值拟合[SplitSum - Part2]
    float NoV = saturate(abs(dot(N, V)) + 1e-5);
